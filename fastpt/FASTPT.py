...
class FASTPT:
    ...
    def compute_term(self, term, X, operation=None, P=None, P_window=None, C_window=None):
        ...
        result = self.cache.get(term, P, P_window, C_window)
        ...
        if isinstance(X, tuple) and all(isinstance(x, tuple) for x in X):
            ...
            if operation:
                ...
                self.cache.set(final_result, term, P, P_window, C_window)
                return final_result
        
            self.cache.set(results, term, P, P_window, C_window)
            return results
        ...
        self.cache.set(result, term, P, P_window, C_window)
        return result
    ...
    def one_loop_dd(self, P, P_window=None, C_window=None): #Acts as its own get function (like IA_der)
        ...
        result = self.cache.get("one_loop_dd", P, P_window, C_window)
        ...
        self.cache.set(P_1loop, "one_loop_dd", P, P_window, C_window)
    ...
    def get_P22(self, P, P_window=None, C_window=None):
        result = self.cache.get("P22", P, P_window, C_window)
        if result is not None: return result
        ...
        self.cache.set(P22, "P22", P, P_window, C_window)
        return P22

    def get_P13(self, P, P_window=None, C_window=None):
        result = self.cache.get("P13", P, P_window, C_window)
        if result is not None: return result
        Ps, _ = self.J_k_scalar(P, self.X_spt, -2, P_window=P_window, C_window=C_window)
        P13 = P_13_reg(self.k_extrap, Ps)
        self.cache.set(P13, "P13", P, P_window, C_window)
        return P13
    ...
    def get_sig4(self, P, P_window=None, C_window=None):
        result = self.cache.get("sig4", P, P_window, C_window)
        if result is not None: return result
        Ps, _ = self.J_k_scalar(P, self.X_spt, -2, P_window=P_window, C_window=C_window)
        sig4 = np.trapz(self.k_extrap ** 3 * Ps ** 2, x=np.log(self.k_extrap)) / (2. * pi ** 2)
        self.cache.set(sig4, "sig4", P, P_window, C_window)
        return sig4

    def get_Pd1d2(self, P, P_window=None, C_window=None):
        result = self.cache.get("Pd1d2", P, P_window, C_window)
        if result is not None: return result
        _, mat = self.J_k_scalar(P, self.X_spt, -2, P_window=P_window, C_window=C_window)
        Pd1d2 = 2. * (17. / 21 * mat[0, :] + mat[4, :] + 4. / 21 * mat[1, :])
        Pd1d2 = self._apply_extrapolation(Pd1d2)
        self.cache.set(Pd1d2, "Pd1d2", P, P_window, C_window)
        return Pd1d2
    
    def get_Pd2d2(self, P, P_window=None, C_window=None):
        result = self.cache.get("Pd2d2", P, P_window, C_window)
        if result is not None: return result
        _, mat = self.J_k_scalar(P, self.X_spt, -2, P_window=P_window, C_window=C_window)
        Pd2d2 = 2. * (mat[0, :])
        Pd2d2 = self._apply_extrapolation(Pd2d2)
        self.cache.set(Pd2d2, "Pd2d2", P, P_window, C_window)
        return Pd2d2
    
    def get_Pd1s2(self, P, P_window=None, C_window=None):
        result = self.cache.get("Pd1s2", P, P_window, C_window)
        if result is not None: return result
        _, mat = self.J_k_scalar(P, self.X_spt, -2, P_window=P_window, C_window=C_window)
        ...
        self.cache.set(Pd1s2, "Pd1s2", P, P_window, C_window)
        return Pd1s2
    
    def get_Pd2s2(self, P, P_window=None, C_window=None):
        result = self.cache.get("Pd2s2", P, P_window, C_window)
        if result is not None: return result
        _, mat = self.J_k_scalar(P, self.X_spt, -2, P_window=P_window, C_window=C_window)
        Pd2s2 = 2. * (2. / 3 * mat[1, :])
        Pd2s2 = self._apply_extrapolation(Pd2s2)
        self.cache.set(Pd2s2, "Pd2s2", P, P_window, C_window)
        return Pd2s2
    
    def get_Ps2s2(self, P, P_window=None, C_window=None):
        result = self.cache.get("Ps2s2", P, P_window, C_window)
        if result is not None: return result
        _, mat = self.J_k_scalar(P, self.X_spt, -2, P_window=P_window, C_window=C_window)
        Pd2s2 = 2. * (4. / 45 * mat[0, :] + 8. / 63 * mat[1, :] + 8. / 35 * mat[2, :])
        Pd2s2 = self._apply_extrapolation(Pd2s2)
        self.cache.set(Pd2s2, "Ps2s2", P, P_window, C_window)
        return Pd2s2
    ...
    def get_sig3nl(self, P, P_window=None, C_window=None):
        result = self.cache.get("sig3nl", P, P_window, C_window)
        if result is not None: return result
        Ps, _ = self.J_k_scalar(P, self.X_spt, -2, P_window=P_window, C_window=C_window)
        sig3nl = Y1_reg_NL(self.k_extrap, Ps)
        sig3nl = self._apply_extrapolation(sig3nl)
        self.cache.set(sig3nl, "sig3nl", P, P_window, C_window)
        return sig3nl
    ...
    def get_Pb1L(self, P, P_window=None, C_window=None):
        result = self.cache.get("Pb1L", P, P_window, C_window)
        if result is not None: return result
        ...
        self.cache.set(Pb1L, "Pb1L", P, P_window, C_window)
        return Pb1L
    
    def get_Pb1L_2(self, P, P_window=None, C_window=None):
        result = self.cache.get("Pb1L_2", P, P_window, C_window)
        if result is not None: return result
        Ps, mat = self.J_k_scalar(P, self.X_lpt, -2, P_window=P_window, C_window=C_window)
        [j000, j002, j2n22, j1n11, j1n13, j004, j2n20] = [mat[0, :], mat[1, :], mat[2, :], mat[3, :], mat[4, :],
                                                          mat[5, :], mat[6, :]]
        ...
        self.cache.set(Pb1L_2, "Pb1L_2", P, P_window, C_window)
        return Pb1L_2

    def get_Pb1L_b2L(self, P, P_window=None, C_window=None):
        result = self.cache.get("Pb1L_b2L", P, P_window, C_window)
        if result is not None: return result
        Ps, mat = self.J_k_scalar(P, self.X_lpt, -2, P_window=P_window, C_window=C_window)
        [j000, j002, j2n22, j1n11, j1n13, j004, j2n20] = [mat[0, :], mat[1, :], mat[2, :], mat[3, :], mat[4, :],
                                                          mat[5, :], mat[6, :]]
        ...
        self.cache.set(Pb1L_b2L, "Pb1L_b2L", P, P_window, C_window)
        return Pb1L_b2L
    
    def get_Pb2L(self, P, P_window=None, C_window=None):
        result = self.cache.get("Pb2L", P, P_window, C_window)
        if result is not None: return result
        Ps, mat = self.J_k_scalar(P, self.X_lpt, -2, P_window=P_window, C_window=C_window)
        [j000, j002, j2n22, j1n11, j1n13, j004, j2n20] = [mat[0, :], mat[1, :], mat[2, :], mat[3, :], mat[4, :],
                                                          mat[5, :], mat[6, :]]
        ...
        self.cache.set(Pb2L, "Pb2L", P, P_window, C_window)
        return Pb2L
    
    def get_Pb2L_2(self, P, P_window=None, C_window=None):
        result = self.cache.get("Pb2L_2", P, P_window, C_window)
        if result is not None: return result
        Ps, mat = self.J_k_scalar(P, self.X_lpt, -2, P_window=P_window, C_window=C_window)
        [j000, j002, j2n22, j1n11, j1n13, j004, j2n20] = [mat[0, :], mat[1, :], mat[2, :], mat[3, :], mat[4, :],
                                                          mat[5, :], mat[6, :]]
        ...
        self.cache.set(Pb2L_2, "Pb2L_2", P, P_window, C_window)
        return Pb2L_2
    ...
    def get_P_Btype2(self, P):
        result = self.cache.get("P_Btype2", P)
        if result is not None: return result
        P_Btype2 = P_IA_B(self.k_original, P)
        P_Btype2 = 4 * P_Btype2
        self.cache.set(P_Btype2, "P_Btype2", P)
        return P_Btype2
    ...
    def get_P_deltaE2(self, P):
        result = self.cache.get("P_deltaE2", P)
        if result is not None: return result
        P_deltaE2 = P_IA_deltaE2(self.k_original, P)
        #Add extrap?
        P_deltaE2 = 2 * P_deltaE2
        self.cache.set(P_deltaE2, "P_deltaE2", P)
        return P_deltaE2
    ...
    def IA_der(self, P, P_window=None, C_window=None):
        result = self.cache.get("IA_der", P, P_window, C_window)
        if result is not None: return result
        self.validate_params(P, P_window=P_window, C_window=C_window)
        P_der = (self.k_original**2)*P
        self.cache.set(P_der, "IA_der", P, P_window, C_window)
        return P_der
    ...
    def get_P_0tE(self, P, P_window=None, C_window=None):
        result = self.cache.get("P_0tE", P, P_window, C_window)
        if result is not None: return result
        nu=-2
        ...
        P_0tE = 2*P_0tE
        self.cache.set(P_0tE, "P_0tE", P, P_window, C_window)
        return P_0tE
    
    def get_P_0EtE(self, P, P_window=None, C_window=None):
        result = self.cache.get("P_0EtE", P, P_window, C_window)
        if result is not None: return result
        ...
        P_0EtE = 2*P_0EtE
        self.cache.set(P_0EtE, "P_0EtE", P, P_window, C_window)
        return P_0EtE
    
    def get_P_E2tE(self, P, P_window=None, C_window=None):
        result = self.cache.get("P_E2tE", P, P_window, C_window)
        if result is not None: return result
        ...
        P_E2tE = 2*P_E2tE
        self.cache.set(P_E2tE, "P_E2tE", P, P_window, C_window)
        return P_E2tE
    
    def get_P_tEtE(self, P, P_window=None, C_window=None):
        result = self.cache.get("P_tEtE", P, P_window, C_window)
        if result is not None: return result
        ...
        P_tEtE = 2*P_tEtE
        self.cache.set(P_tEtE, "P_tEtE", P, P_window, C_window)
        return P_tEtE
    ...
    def _cache_fourier_coefficients(self, P_b, C_window=None):
        ...
        result = self.cache.get("fourier_coefficients", P_b, C_window)
        if result is not None: return result
        ...
        self.cache.set(c_m, "fourier_coefficients", P_b, C_window)
        return c_m

    def _cache_convolution(self, c1, c2, g_m, g_n, h_l, two_part_l=None):
        """Cache and return convolution results"""
        result = self.cache.get("convolution", c1, c2, g_m, g_n, h_l, two_part_l)
        if result is not None: return result
        ...
        # Cache and return
        self.cache.set(C_l, "convolution", c1, c2, g_m, g_n, h_l, two_part_l)
        return C_l


    def J_k_scalar(self, P, X, nu, P_window=None, C_window=None):
        result = self.cache.get("J_k_scalar", P, X, nu, P_window, C_window)
        if result is not None: return result
        ...
        self.cache.set((P_out, A_out), "J_k_scalar", P, X, nu, P_window, C_window)
        return P_out, A_out

    
    def J_k_tensor(self, P, X, P_window=None, C_window=None):
        result = self.cache.get("J_k_tensor", P, X, P_window, C_window)
        if result is not None: return result
        ...
        self.cache.set((P_fin, A_out), "J_k_tensor", P, X, P_window, C_window)
        return P_fin, A_out
