...
t_init_start = time()
fpt = FASTPT(k, low_extrap=-5, high_extrap=3, n_pad=100, verbose=False)
t_init_end = time()
...
# Generate a power spectrum using CAMB (falls back to simple power law if CAMB not available)
...
t2 = time()
print(f'  → one_loop_dd: {t2-t1:.3f} seconds')

t1 = time()
ia_result = handler.run('IA_tt')
t2 = time()
print(f'  → IA_tt: {t2-t1:.3f} seconds')

t1 = time()
bias_result = handler.run('one_loop_dd_bias')
t2 = time()
print(f'  → one_loop_dd_bias: {t2-t1:.3f} seconds')
...
t2 = time()
print(f'  → one_loop_dd: {t2-t1:.3f} seconds (cached!)')

t1 = time()
ia_result2 = handler.run('IA_tt')
t2 = time()
print(f'  → IA_tt: {t2-t1:.3f} seconds (cached!)')
...
t1 = time()
ov_result = handler.run('OV')
t2 = time()
print(f'  → OV: {t2-t1:.3f} seconds (fast due to shared matrices)')
...
# Plot 4: Comparison plot with ratio panel
if 'P_alt' in locals():
    # Calculate 1-loop for alternative cosmology
    P_1loop_alt = handler.get('P_1loop', P=P_alt)
    
    fig4 = handler.plot_comparison(
        results_dict={
            'Fiducial cosmology': P_linear + P_1loop,
            'Alternative cosmology': P_alt + P_1loop_alt
        },
        ratio=True,
        ratio_baseline='Fiducial cosmology',
        title='Cosmology Comparison with Ratio',
        return_fig=True,
        show=False
    )
